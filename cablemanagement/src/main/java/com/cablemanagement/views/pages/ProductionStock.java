...
public class ProductionStock {
    ...
    private static TableView<ProductionStockRecord> createProductionStockTable() {
        ...
        salePriceCol.setCellValueFactory(data -> new SimpleStringProperty(String.format("$%.2f", data.getValue().getSalePrice())));
        salePriceCol.setPrefWidth(100);
        
        TableColumn<ProductionStockRecord, String> profitCol = new TableColumn<>("Profit");
        profitCol.setCellValueFactory(data -> {
            double unitCost = data.getValue().getUnitCost();
            double salePrice = data.getValue().getSalePrice();
            double profit = salePrice - unitCost;
            String profitText = String.format("$%.2f", profit);
            return new SimpleStringProperty(profitText);
        });
        profitCol.setPrefWidth(80);
        
        table.getColumns().addAll(nameCol, brandCol, quantityCol, unitCostCol, salePriceCol, profitCol);
        return table;
    }
